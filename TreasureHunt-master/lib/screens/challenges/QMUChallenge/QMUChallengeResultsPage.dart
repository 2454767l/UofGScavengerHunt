...
class OurKGChallengeResultsPage extends StatelessWidget {
  const OurKGChallengeResultsPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    KGChallengeController _qnController = Get.put(KGChallengeController());
    ...
    void _updateTreasureAndGoHome(BuildContext context) async {
      ...
      //update the users QMUComletionStatus
      await OurDatabase().updateQMUChallengeCompletionStatus(_firebaseUser.uid);
          ...
          child: Column(
            children: [
              ...
              const Padding(
                padding: EdgeInsets.all(standardPadding / 2),
                child: Text(
                  "You Have Completed the Kelvin Grove Challenge",
                  style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: goldenText,
                      fontSize: 16),
